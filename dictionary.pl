...
sub threshold_clasification{
    ...
    if($THRESHOLD == 0.5 && $hash_ngram =~ /tekno/){
        print OUTTEKNO50 $word.":".$wordTekno{$word}.":".sprintf("%.8f",($wordTekno{$word}/$total{$hash_ngram}))."\n";
    }elsif($THRESHOLD == 0.5 && $hash_ngram =~ /travel/){
        print OUTTRAVEL50 $word.":".$wordTravel{$word}.":".sprintf("%.8f",($wordTravel{$word}/$total{$hash_ngram}))."\n";
    }elsif($hash_ngram =~ /tekno/){
        print OUTTEKNO45 $word.":".$wordTekno{$word}.":".sprintf("%.8f",($wordTekno{$word}/$total{$hash_ngram}))."\n";
    }else{
        print OUTTRAVEL45 $word.":".$wordTravel{$word}.":".sprintf("%.8f",($wordTravel{$word}/$total{$hash_ngram}))."\n";
    }
}